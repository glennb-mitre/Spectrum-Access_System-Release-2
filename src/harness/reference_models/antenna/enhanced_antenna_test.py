...
class TestEnhancedAntenna(unittest.TestCase):
  ...
  def test_get_standard_gains_hor(self):
      ...
      gains = antenna.get_standard_antenna_gain(dirs,ant_azimuth, ant_beamwidth)
      for k, hor in enumerate(dirs['hor']):
        dir = {'hor': hor}
        gain = antenna.get_standard_antenna_gain(dir, ant_azimuth, ant_beamwidth)
      ...
      ant_beamwidth = None
          ...
          # Isotropic antenna
      gains = antenna.get_standard_antenna_gain(dirs, ant_azimuth, ant_beamwidth)
      self.assertEqual(np.max(np.abs(gains)), 0)
      ...
      gains = antenna.get_standard_antenna_gain(dirs, None, ant_beamwidth)
      self.assertEqual(np.max(np.abs(gains)), 0)
      ...
      gain = antenna.get_standard_antenna_gain(dirs, ant_azimuth, ant_beamwidth)
  ...
  def test_get_standard_gains_hor_ver(self):
      ...
      [hor_gain,ver_gain] = antenna.get_standard_antenna_gains_hor_ver(dirs_relative_boresight, ant_azimuth = ant_az, ant_mech_downtilt = downtilt,
                                                          ant_hor_beamwidth = beamwidth_hor, ant_ver_beamwidth = beamwidth_ver)
      ...
      np.testing.assert_array_equal([hor_gain,ver_gain],[0,0])
      ...
      np.testing.assert_array_equal([hor_gain,ver_gain],[0-3,0-3])
      ...
      np.testing.assert_array_equal([hor_gain,ver_gain],[-3,-3])
