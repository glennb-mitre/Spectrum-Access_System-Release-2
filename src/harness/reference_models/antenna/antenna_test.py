...
class TestAntenna(unittest.TestCase):
  ...
  def test_pattern_gain(self):
    ...
    gain = antenna.calculate_gain_from_hor_pattern(40, 0, pattern)
    self.assertAlmostEqual(gain, 40)
    gain = antenna.calculate_gain_from_hor_pattern(40.5, 0, pattern)
    self.assertAlmostEqual(gain, 40.5)
    gain = antenna.calculate_gain_from_hor_pattern(40.2, 20, pattern)
    self.assertAlmostEqual(gain, 20.2)
    hor_dirs = [10.5, 42.2, 25]
    gains = antenna.calculate_gain_from_hor_pattern(hor_dirs, 0, pattern)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([10.5, 42.2, 25]))), 0)
    gains = antenna.calculate_gain_from_hor_pattern(hor_dirs, 20, pattern)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([350.5, 22.2, 5]))), 0)
    gains = antenna.calculate_gain_from_hor_pattern(hor_dirs, 20, pattern, 10)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([360.5, 32.2, 15]))), 0)
    # Vectorized vs scalar
    hor_dirs = [3.5, 47.3, 342]
    gains = antenna.calculate_gain_from_hor_pattern(hor_dirs, 123.3, pattern, 12.4)
    for k, hor in enumerate(hor_dirs):
      gain = antenna.calculate_gain_from_hor_pattern(hor, 123.3, pattern, 12.4)
  ...
  def test_dpa_gain(self):
    ...
    gains = antenna.get_radar_normalized_gains(hor_dirs, 0)
    self.assertEqual(np.max(np.abs(
        gains - np.array([0, 0, 0, -25, -25, -25, -25]))), 0)
    for k, hor in enumerate(hor_dirs):
      gain = antenna.get_radar_normalized_gains(hor, 0)
      self.assertEqual(gain, gains[k])

    gains = antenna.get_radar_normalized_gains([5, 8.6, 11.4, 20], 10)
    self.assertEqual(np.max(np.abs(
        gains - np.array([-25, 0, 0, -25]))), 0)

    gains = antenna.get_radar_normalized_gains([5, 8.99, 9.01, 10, 10.99, 11.01],
  ...
  def test_fss_gain(self):
    ...
    #   * perpendicular
    gains = antenna.get_fss_gains(hor_dirs, ver_dirs,
                                  20, 0, 30, 0, 1)
    self.assertAlmostEqual(np.max(np.abs(
    ...
    gains = antenna.get_fss_gains(ver_dirs, hor_dirs,
                                  0, 20, 30, 0, 1)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([30, 30, 30, 30, 30,
                          32-25*np.log10(3.1),
