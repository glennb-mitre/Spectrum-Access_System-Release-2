...
class TestAntenna(unittest.TestCase):
  ...
  def test_pattern_gain(self):
    ...
    gain = antenna.get_given_1d_pattern_gains(40, 0, pattern)
    self.assertAlmostEqual(gain, 40)
    gain = antenna.get_given_1d_pattern_gains(40.5, 0, pattern)
    self.assertAlmostEqual(gain, 40.5)
    gain = antenna.get_given_1d_pattern_gains(40.2, 20, pattern)
    self.assertAlmostEqual(gain, 20.2)
    hor_dirs = [10.5, 42.2, 25]
    gains = antenna.get_given_1d_pattern_gains(hor_dirs, 0, pattern)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([10.5, 42.2, 25]))), 0)
    gains = antenna.get_given_1d_pattern_gains(hor_dirs, 20, pattern)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([350.5, 22.2, 5]))), 0)
    gains = antenna.get_given_1d_pattern_gains(hor_dirs, 20, pattern, 10)
    self.assertAlmostEqual(np.max(np.abs(
        gains - np.array([360.5, 32.2, 15]))), 0)
    # Vectorized vs scalar
    hor_dirs = [3.5, 47.3, 342]
    gains = antenna.get_given_1d_pattern_gains(hor_dirs, 123.3, pattern, 12.4)
    for k, hor in enumerate(hor_dirs):
      gain = antenna.get_given_1d_pattern_gains(hor, 123.3, pattern, 12.4)
